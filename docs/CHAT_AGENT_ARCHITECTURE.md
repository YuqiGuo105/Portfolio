# Chat Agent Architecture：面向 AI Framework 的高层架构设计

> Repository: `YuqiGuo105/Portfolio`  
> Branch: `docs/ai-agent-architecture`  
> Scope: Portfolio ChatWidget、RAG、Agent Orchestration、MCP Tools、Telemetry、Evaluation、Analytics  
> Language: Chinese
...
- 前端保持轻量：负责 ChatWidget UI、SSE 消费、文件上传、页面上下文提取、telemetry 上报；
...
> 版本说明：具体框架版本应在实现时重新确认。本设计重点是让系统能持续接入新的 AI framework，而不是绑定某个固定版本。
...
| 编号 | 服务 | 推荐技术 | 核心职责 |
|---|---|---|---|
| S1 | Frontend Service | Next.js 12 / React 18 / AI SDK-compatible client | ChatWidget、SSE 消费、上传文件、页面上下文、telemetry |
...
| S5 | Evaluation Service | Spring Boot / Python Worker + LLM-as-Judge | 异步质量评分、事实性检测、样本标注 |
...
| 当前模式 | 目标模式 |
|---|---|
| 前端 `finalizeAndPersist` 同步写 Supabase | 后端 S3 统一写 PostgreSQL |
...
1. 主栈是 Java / Spring Boot，和后端 SDE 方向一致；
...
4. Python / JS 的 agent framework 变化很快，不建议直接耦合在主请求链路；
...
| Key | TTL | 说明 |
|---|---:|---|
| session metadata | 24h | 支持当天继续上下文 |
| recent messages | 1h–24h | 根据隐私策略调整 |
| dedup key | 5–10min | 防止浏览器重试重复写入 |
| rate counter | 1min | 10 req/min/session |
| realtime counters | rolling | Dashboard 实时指标 |
...
create index if not exists idx_chat_sessions_last_active
  on public.chat_sessions(last_active_at desc);
```
...
create unique index if not exists idx_chat_messages_client_event
  on public.chat_messages(client_event_id)
...
create index if not exists idx_chat_messages_trace
  on public.chat_messages(trace_id);
```
...
```mermaid
flowchart LR
  K[Kafka chat-events] --> E[Evaluation Service]
  E --> J[LLM-as-Judge]
  E --> R[Ragas/DeepEval Optional]
  J --> S[QualityScore]
  R --> S
  S --> PG[(PostgreSQL)]
  S --> K2[Kafka quality-scores]
```
...
evaluation.consume
  ├── judge.prompt_build
  ├── judge.model_invoke
  ├── postgres.insert_quality_score
  └── kafka.produce.quality_score
```
...
| Metric | 类型 | 说明 |
|---|---|---|
| `chat_requests_total` | Counter | 请求量 |
| `chat_errors_total` | Counter | 错误量 |
| `chat_latency_ms` | Histogram | 总延迟 |
| `chat_first_token_latency_ms` | Histogram | 首 token 延迟 |
| `llm_tokens_total` | Counter | token 使用量 |
| `rag_hit_rate` | Gauge | RAG 命中率 |
| `quality_score_overall` | Gauge | 平均质量分 |
| `tool_call_latency_ms` | Histogram | MCP 工具调用延迟 |
| `kafka_consumer_lag` | Gauge | Kafka 消费滞后 |
| `sse_active_connections` | Gauge | 当前 SSE 连接数 |

### 11.3 可选 AI Observability 工具
...
sequenceDiagram
  ...
  participant S1 as S1 Frontend ChatWidget
  participant S2 as S2 API Gateway
  participant S3 as S3 Chat Orchestrator
  participant S4 as S4 Redis Session
  ...
  participant S5 as S5 Evaluation
  participant K2 as Kafka quality-scores
  participant S6 as S6 Analytics
  participant S7 as S7 Notification
  participant OTel as OpenTelemetry

  User->>S1: 输入问题
  S1->>S1: 生成 traceId/sessionId/messageId/clientEventId
  S1->>S2: POST /api/rag/answer/stream
  S2->>S2: Redis rate limit: 10 req/min/session
  S2->>S3: 转发请求 + trace headers

  S3->>OTel: start chat.request span
  S3->>S3: ValidatorPlugin 校验/脱敏/去重
  S3->>S4: load session
  S4-->>S3: session state
  S3->>RAG: retrieve portfolio/context chunks
  RAG-->>S3: ranked sources
  S3->>MCP: optional tool calls
  MCP-->>S3: tool results
  S3->>LLM: stream model response
  LLM-->>S3: tokens
  S3-->>S1: answer_delta
  S1-->>User: 实时展示

  S3->>S4: save session state
  S3->>PG: insert chat_messages
  S3->>K: produce ChatEvent
  S3-->>S1: answer_final

  K-->>S5: consume ChatEvent
  S5->>LLM: LLM-as-Judge
  S5->>PG: insert chat_quality_scores
  S5->>K2: produce QualityScore

  K2-->>S6: update analytics
  K2-->>S7: check alert rules
```
...
如果不想把后端放进 Portfolio repo，也可以保留 Portfolio 作为前端 repo，另建：

```text
YuqiGuo105/ai-agent-platform
```

然后 Portfolio 只通过 environment variable 指向后端：
