...
import { ensurePermission, permissionedFetch } from "../../os/ipc/gateway";
...
const ProjectIsotop = dynamic(() => import("../../components/ProjectIsotop"), {
  ssr: false,
});
...
const Index = () => {
  ...
  useEffect(() => {
    const bootstrap = async () => {
      try {
        await ensurePermission('net', 'Allow the Home app to fetch portfolio content from Supabase?');
      } catch (err) {
  ...
  useEffect(() => {
    const fetchStories = async () => {
      try {
        ...
        const res      = await permissionedFetch(`${endpoint}/records/owner/${owner}`);
